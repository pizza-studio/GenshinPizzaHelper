    ...
    var body: some View {
        List {
            Section {
                ...
                HStack {
                    Text("\(gachaViewModel.filter.gachaType.localizedDescription())五星祈愿抽数")
                    Spacer()
                    Button("切换卡池") {
                        withAnimation {
                            switch gachaViewModel.filter.gachaType {
                            case .character: gachaViewModel.filter.gachaType = .weapon
                            case .weapon: gachaViewModel.filter.gachaType = .standard
                            case .standard: gachaViewModel.filter.gachaType = .character
                            default: gachaViewModel.filter.gachaType = .character
                            }
                        }
                    }
                    .font(.caption)
            ...
            Section {
                GachaTimeChart(type: gachaViewModel.filter.gachaType, items: gachaViewModel.gachaItems)
            } header: {
                Text("总抽数分布")
            }
        }
...
//                    y: .value("抽数", item.count)
            ...
            RuleMark(x: .value("平均", fiveStarItems.map({$0.count}).reduce(0, {$0 + $1}) / fiveStarItems.count))
                .foregroundStyle(.gray)
                .lineStyle(StrokeStyle(lineWidth: 2, dash: [5]))
                .annotation(alignment: .topLeading) {
                    Text("平均抽数：\(fiveStarItems.map({$0.count}).reduce(0, {$0 + $1}) / fiveStarItems.count)").font(.caption).foregroundColor(.gray)
                }
        }
        .chartYAxis(content: {
            AxisMarks { value in
...
private struct GachaTimeChart: View {
    ...
    var data: [GachaTypeDateCount] {
        ...
        return types.flatMap { type in
            dates.map { date in
                GachaTypeDateCount.init(date: date, count: items.filter({($0.time <= date) && ($0.gachaType == type)}).count, type: type)
