                    ...
                    Section {
                        Button("从粘贴板中获取祈愿URL") {
                            if let str = UIPasteboard.general.string {
                                if case .success =
                                    parseURLToAuthkeyAndOtherParams(
                                        urlString: str
                                    ) {
                                    withAnimation {
                                        urls.append(str)
                    ...
                    Button(urls.isEmpty ? "等待祈愿链接..." : "开始获取祈愿记录") {
                        ...
                        if parseResults.isEmpty {
                            withAnimation {
                                status =
                                    .failure(
                                        .genAuthKeyError(message: "URL Error")
                                    )
                            }
                        } else {
...
//    }
    ...
    private func parseURLToAuthkeyAndOtherParams(urlString: String)
        ...
        guard let url = URL(string: urlString)
            ...
            return .failure(
                .genAuthKeyError(message: "URL Error: \(urlString)")
            )
        }
        ...
        guard let authkey = items.first(where: { $0.name == "authkey" })?.value
            ...
            return .failure(
                .genAuthKeyError(
                    message: "URL ERROR (no authkey): \(urlString)"
                )
            )
        }
        guard let signTypeString = items
            .first(where: { $0.name == "sign_type" })?
            .value
        else {
            return .failure(
                .genAuthKeyError(
                    message: "URL ERROR (no sign_type): \(urlString)"
                )
            )
        }
