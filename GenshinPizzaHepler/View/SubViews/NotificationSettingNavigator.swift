...
// MARK: - NotificationSettingNavigatorIOS16

@available(iOS 16.0, *)
struct NotificationSettingNavigatorIOS16: View {
    @Binding
    ...
    var body: some View {
        Section {
            ...
            .disabled(!allowNotification)
            if masterSwitch.wrappedValue, allowNotification {
                NavigationLink(value: SettingViewIOS16.Navigation.notificationSetting) {
                    Label("settings.notification.deliverySettings", systemSymbol: .appBadgeFill)
                }
                .animation(.easeInOut, value: masterSwitch.wrappedValue)
            }
