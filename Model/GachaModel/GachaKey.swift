...
public extension MihoyoAPI {
    static func genAuthKey(
        ...
        let genAuthKeyParam = GenAuthKeyParam(gameUid: account.uid!, region: account.server.id, gameBiz: gameBiz)
...
//        }
        ...
        URLSession.shared.dataTask(with: request) { data, respond, error in
            ...
            guard error == nil else { completion(.failure(.genAuthKeyError(message: error!.localizedDescription))); return }
            ...
            do {
                ...
                completion(.failure(.genAuthKeyError(message: error.localizedDescription)))
            }
        }.resume()
        func get_ds_token() -> String {
            ...
            let r = String((0..<6).map({_ in lettersAndNumbers.randomElement()!}))
...
//            print(String(data: data!, encoding: .utf8))
            ...
            decoder.keyDecodingStrategy = .convertFromSnakeCase
            do {
                let result = try decoder.decode(GetCookieTokenResult.self, from: data!)
                if let data = result.data {
                    completion(.success((data.cookieToken, data.uid)))
                } else {
                    completion(.failure(.genAuthKeyError(message: result.message)))
                }
            } catch {
                completion(.failure(.genAuthKeyError(message: error.localizedDescription)))
            }
...
fileprivate struct GetCookieTokenResult: Codable {
    let retcode: Int
    let message: String
...
fileprivate struct GenAuthKeyParam: Encodable {
