...
public struct PieChartView: View {
    ...
    var slices: [PieSliceData] {
        ...
        for (i, value) in values.enumerated() {
            ...
            if degrees < 18 {
                tempSlices.append(PieSliceData(startAngle: Angle(degrees: endDeg), endAngle: Angle(degrees: endDeg + degrees), text: String(format: "%.0f%%", value * 100 / sum), color: self.colors[i], isIgnored: true))
            } else {
