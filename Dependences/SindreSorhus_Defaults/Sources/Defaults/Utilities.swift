...
final class LifetimeAssociation {
     ...
     */
    init(of target: AnyObject, with owner: AnyObject, deinitHandler: @escaping () -> () = {}) {
    ...
    private class ObjectLifetimeTracker {
        ...
        init(for weaklyHeldObject: AnyObject, deinitHandler: @escaping () -> ()) {
...
// MARK: - _DefaultsOptionalProtocol
...
// MARK: - Optional + _DefaultsOptionalProtocol
...
extension Defaults.Serializable {
    ...
    static func toValue<T: Defaults.Serializable>(_ anyObject: Any, type: T.Type = Self.self) -> T? {
        ...
        guard let nextType = T.Serializable.self as? any Defaults.Serializable.Type,
              nextType != T.self
        else {
