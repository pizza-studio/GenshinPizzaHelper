...
extension Defaults {
    ...
    /**
     ...
     ```
     */
    public static func withoutPropagation(_ closure: () -> ()) {
        // How does it work?
    ...
    final class UserDefaultsKeyObservation: NSObject, Observation {
        ...
        override func observeValue(
            ...
            guard selfObject == object as? NSObject,
                  let change
            else {
                return
            }
    ...
    private final class CompositeUserDefaultsKeyObservation: NSObject, Observation {
        ...
        init(
            observables: [(suite: UserDefaults, key: String)],
            callback: @escaping UserDefaultsKeyObservation.Callback
        ) {
    ...
    public static func observe<Value: Serializable>(
        ...
        handler: @escaping (KeyChange<Value>) -> ()
    )
        -> Observation {
...
// MARK: - Defaults.KeyChange + Equatable
