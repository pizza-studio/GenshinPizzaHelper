...
final class DefaultsTests: XCTestCase {
    ...
    func testDynamicOptionalDateType() {
        ...
        XCTAssertEqual(
            UserDefaults.standard.object(forKey: Defaults.Key<Date>.defaultDynamicOptionalDate.name) as! Date,
            next
        )
    ...
    func testObserveWithLifetimeTieManualBreak() {
        ...
        weak var observation: Defaults.Observation? = Defaults.observe(key, options: []) { _ in }
            .tieToLifetime(of: self)
    ...
    func testKeyEquatable() {
        XCTAssertEqual(
            Defaults.Key<Bool>("equatableKeyTest", default: false),
            Defaults.Key<Bool>("equatableKeyTest", default: false)
        )
    }
