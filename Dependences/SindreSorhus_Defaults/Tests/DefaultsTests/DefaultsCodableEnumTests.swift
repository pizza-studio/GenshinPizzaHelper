...
// MARK: - FixtureCodableEnumPreferRawRepresentable

private enum FixtureCodableEnumPreferRawRepresentable: Int, Hashable, Codable, Defaults.Serializable,
...
extension Defaults.Keys {
    ...
    fileprivate static let codableEnumDictionary = Key<[String: FixtureCodableEnum]>(
        "codable_enum",
        default: ["0": .oneHour]
    )
}

// MARK: - DefaultsCodableEnumTests
...
final class DefaultsCodableEnumTests: XCTestCase {
    ...
    func testArrayDictionaryKey() {
        let key = Defaults
            .Key<[[String: FixtureCodableEnum]]>(
                "independentCodableEnumArrayDictionaryKey",
                default: [["0": .tenMinutes]]
            )
        XCTAssertEqual(Defaults[key][0]["0"], .tenMinutes)
    ...
    func testDictionaryKey() {
        let key = Defaults
            .Key<[String: FixtureCodableEnum]>("independentCodableEnumDictionaryKey", default: ["0": .tenMinutes])
        XCTAssertEqual(Defaults[key]["0"], .tenMinutes)
    ...
    func testDictionaryArrayKey() {
        let key = Defaults
            .Key<[String: [FixtureCodableEnum]]>(
                "independentCodableEnumDictionaryArrayKey",
                default: ["0": [.tenMinutes]]
            )
        XCTAssertEqual(Defaults[key]["0"]?[0], .tenMinutes)
    ...
    func testObserveKeyCombine() {
        ...
        let expectedValue: [(FixtureCodableEnum, FixtureCodableEnum)] = [
            (.tenMinutes, .oneHour),
            (.oneHour, .tenMinutes),
        ]
    ...
    func testObserveDictionaryKeyCombine() {
        let key = Defaults
            .Key<[String: FixtureCodableEnum]>("observeCodableEnumDictionaryKeyCombine", default: ["0": .tenMinutes])
        let expect = expectation(description: "Observation closure being called")
    ...
    func testObserveDictionaryKey() {
        let key = Defaults
            .Key<[String: FixtureCodableEnum]>("observeCodableEnumDictionaryKey", default: ["0": .tenMinutes])
        let expect = expectation(description: "Observation closure being called")
