...
final class DefaultsDictionaryTests: XCTestCase {
    ...
    func testNestedKey() {
        let key = Defaults
            .Key<[String: [String: String]]>("independentDictionaryNestedKey", default: ["0": fixtureDictionary])
    ...
    func testObserveKeyCombine() {
        ...
        let cancellable = publisher.sink { tuples in
            for (index, expected) in [(fixtureDictionary["0"]!, newName), (newName, fixtureDictionary["0"]!)]
                .enumerated() {
    ...
    func testObserveOptionalKeyCombine() {
        ...
        let expectedValues: [([String: String]?, [String: String]?)] = [
            (nil, fixtureDictionary),
            (fixtureDictionary, newName),
            (newName, nil),
        ]
