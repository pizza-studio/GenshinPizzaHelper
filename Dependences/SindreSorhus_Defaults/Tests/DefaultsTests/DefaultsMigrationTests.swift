...
private struct MySet<Element: Defaults.NativeType & Hashable>: SetForm, Defaults.SetAlgebraSerializable,
    Defaults.NativeType {
    // MARK: Lifecycle
...
private struct CodableSet<Element: Defaults.Serializable & Codable & Hashable>: SetForm,
...
private func setCodable(forKey keyName: String, data: some Codable) {
    guard let text = try? JSONEncoder().encode(data),
...
final class DefaultsMigrationTests: XCTestCase {
    ...
    func testArrayAndCodableElementToNativeSetAlgebraType() {
        ...
        XCTAssertEqual(
            Set([TimeZone(id: "0", name: "Asia/Taipei"), TimeZone(id: "1", name: newName)]),
            Defaults[key]?.store
        )
    }
