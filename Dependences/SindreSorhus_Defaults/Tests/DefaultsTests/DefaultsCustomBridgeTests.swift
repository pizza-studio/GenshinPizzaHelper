...
extension Defaults.Keys {
    ...
    fileprivate static let customBridgeDictionary = Key<[String: User]>(
        "dictionary_customBridge",
        default: ["0": fixtureCustomBridge]
    )
}

// MARK: - DefaultsCustomBridge
...
final class DefaultsCustomBridge: XCTestCase {
    ...
    func testNestedArrayKey() {
        let key = Defaults
            .Key<[[User]]>(
                "independentCustomBridgeNestedArrayKey",
                default: [[fixtureCustomBridge], [fixtureCustomBridge]]
            )
    ...
    func testArrayDictionaryKey() {
        let key = Defaults.Key<[[String: User]]>(
            "independentCustomBridgeArrayDictionaryKey",
            default: [["0": fixtureCustomBridge], ["0": fixtureCustomBridge]]
        )
        XCTAssertEqual(Defaults[key][0]["0"]?.username, fixtureCustomBridge.username)
        let newUser = User(username: "sindresorhus", password: "123456789")
    ...
    func testDictionaryKey() {
        let key = Defaults
            .Key<[String: User]>("independentCustomBridgeDictionaryKey", default: ["0": fixtureCustomBridge])
    ...
    func testDictionaryArrayKey() {
        let key = Defaults
            .Key<[String: [User]]>("independentCustomBridgeDictionaryArrayKey", default: ["0": [fixtureCustomBridge]])
    ...
    func testObserveOptionalKeyCombine() {
        ...
        let expectedValue: [(User?, User?)] = [
            (nil, fixtureCustomBridge),
            (fixtureCustomBridge, newUser),
            (newUser, nil),
        ]
    ...
    func testObserveArrayKeyCombine() {
        ...
        let cancellable = publisher.sink { tuples in
            for (index, expected) in [([fixtureCustomBridge], [newUser]), ([newUser], [newUser, fixtureCustomBridge])]
                .enumerated() {
                XCTAssertEqual(expected.0, tuples[index].0)
                XCTAssertEqual(expected.1, tuples[index].1)
            }
    ...
    func testObserveDictionaryCombine() {
        let key = Defaults
            .Key<[String: User]>("observeCustomBridgeDictionaryKeyCombine", default: ["0": fixtureCustomBridge])
        let newUser = User(username: "sindresorhus", password: "123456789")
        let expect = expectation(description: "Observation closure being called")
